...
macOS 使用独立的 SwiftUI `Settings` 窗口，可从 Zero Mail 菜单的「设置…」、`⌘,` 或侧栏打开。顶部按「通用、账户、分类、查看、通知、设备」分类，阅读和显示偏好使用对齐的原生表单；账户、分类和设备列表可滚动。

iOS / iPadOS 将常用偏好放在系统「设置 → App → Zero Mail」中（较早系统直接在「设置 → Zero Mail」）。应用内「设置 → 打开系统设置」使用 Apple 的公开设置入口。账户管理、AI 邮件分类、资源限制、设备配对、撤销、通知授权和通知测试仍在应用内；Watch 使用同一套紧凑表单。

## AI 分类与资源限制

收件箱提供「所有、主要、交易、更新、推广」筛选。分类来自服务器本地缓存；长按列表邮件、阅读工具栏或详情顶部均可手动纠正，手动结果沿用网页端的保护规则，不会被后续自动分类覆盖。

Apple 设置显示跨邮箱分类进度、分类器在线状态和每个邮箱的错误，可开始、继续、暂停或重新分类。并发数、批次大小、批次间隔、请求超时、最近邮件天数及分类历史频率使用网页端相同的范围校验，保存到配对用户的服务器记录，因此所有 Apple 设备与网页端看到同一配置。

模型调用的并发门控、超时、响应大小限制和认证/追踪资源释放仍由服务器执行。Apple 客户端只提交严格校验的数值和邮件分类操作，不接收模型密钥、用户 ID 或服务端资源句柄。

...
Settings follow Apple Mail conventions: macOS has a separate Settings window with General, Accounts, Viewing, Notifications and Devices tabs; iPhone and iPad expose list, reading and notification preferences in the system Settings app through `Settings.bundle`. Changes apply when returning to the running app. Pairing and account management remain in-app. See [Apple settings](SETTINGS.zh-CN.md).

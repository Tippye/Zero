...
export function parseCategories(content, count) {
  ...
  for (const item of data.results) {
    if (!item || !Number.isInteger(item.id) || item.id < 0 || item.id >= count || ids.has(item.id) || !categoryIds.includes(item.category)) throw failure('INVALID_RESPONSE');
    ids.add(item.id);
  }
  return data.results;
}
...
export async function classifyMessages(profile, rows, request = fetch, preferences = [], signal, timeoutMs = 60000) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    response = await request(`${profile.baseUrl}/chat/completions`, {
    ...
    if (structured && /response_format|json_object/i.test(param)) {
      await response.body?.cancel(); structured = false; continue;
    }
    break;
  }
...
export async function classifyAccount(sql, config, account, request = fetch, priority = 'recent') {
  ...
  try {
    ...
    // Pause survives browser closure and aborts the active HTTP request within a second.
    poll = setInterval(async () => {
      if (polling) return;
      polling = true;
      try {
        const [current] = await sql`SELECT classification_paused,classification_generation FROM mail0_sync_accounts WHERE user_id=${owner} AND account_id=${id}`;
        if (!current || current.classification_paused || current.classification_generation !== generation) controller.abort();
    ...
    await sql.begin(async tx => {
      const [active] = await tx`SELECT classification_paused,classification_generation FROM mail0_sync_accounts WHERE user_id=${owner} AND account_id=${id} FOR NO KEY UPDATE`;
      if (!active || active.classification_paused || active.classification_generation !== generation) return;
    ...
    clearInterval(poll);
    await sql`UPDATE mail0_sync_accounts SET classification_running_at=NULL WHERE user_id=${owner} AND account_id=${id} AND classification_generation=${generation}`;
  }
}
